...
WnbdProcessDeviceThreadReplies(_In_ PSCSI_DEVICE_INFORMATION DeviceInformation)
{
    ...
    if (Reply.Error) {
        ...
        // TODO: abort other subrequests when one of them fails (e.g. unmap),
    ...
    InterlockedIncrement64(Element->CompletedRequestCount);
...
Exit:
    ...
    if (Element) {
        if(!Element->Aborted && Element->RequestCount == Element->CompletedRequestCount) {
            WNBD_LOG_INFO("Notifying StorPort of completion of %p status: 0x%x(%s)",
    ...
    _In_ PSRB_QUEUE_ELEMENT Element)
{
    ...
    switch (NbdReqType) {
    case NBD_CMD_FLUSH:
        if(!CHECK_NBD_SEND_TRIM(DeviceInformation->UserEntry->NbdFlags)) {
            WNBD_LOG_LOUD(
                "The NBD server doesn't accept TRIM/UNMAP.",
                Status, Element->Srb, Element->Tag);
            return FALSE;
        }
    case NBD_CMD_WRITE:
