    ...
    case IRP_MN_START_DEVICE:
        {
            ...
            if (Device) {
                ...
                if (AttachedDisk != DeviceObject) {
                    ...
                    if (Status) {
                        WNBD_LOG_WARN("Could not get disk %s number PDO type: %d. Error: %d.",
                                      Device->Properties.InstanceName, DeviceObject->DeviceType, Status);
                    }
                    else {
                        WnbdGetDiskInstancePath(DeviceObject);
                    }
                }
                else {
                    WNBD_LOG_WARN("Couldn't not get disk numbe.");
                }
