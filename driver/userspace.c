                     ...
                     PWNBD_DISK_HANDLE DiskHandle)
{
    ...
    *DiskHandle = WNBD_DISK_HANDLE_FROM_ADDR(BusId, TargetId, LunId);
    WNBD_LOG_INFO("Bus: %d, target: %d, lun: %d, created handle: %llu.",
                  BusId, TargetId, LunId, *DiskHandle);

    NewEntry->ScsiInformation = ScsiInfo;
    NewEntry->BusIndex = BusId;
    NewEntry->TargetIndex = TargetId;
    NewEntry->LunIndex = LunId;
    NewEntry->DiskHandle = *DiskHandle;
                   ...
                   PIRP Irp)
{
    ...
    case IOCTL_WNBD_CREATE:
        ...
        if (!Command ||
            ...
            CHECK_O_LOCATION(IoLocation, WNBD_DISK_HANDLE))
        {
        ...
        WNBD_DISK_HANDLE DiskHandle = 0;
        Status = WnbdCreateConnection(GInfo, &Props, &DiskHandle);

        PWNBD_DISK_HANDLE OutHandle = (PWNBD_DISK_HANDLE) Irp->AssociatedIrp.SystemBuffer;
        RtlCopyMemory(OutHandle, &DiskHandle, sizeof(WNBD_DISK_HANDLE));
        Irp->IoStatus.Information = sizeof(WNBD_DISK_HANDLE);

        WNBD_LOG_LOUD("Mapped disk. Name: %s, Handle: %llu",
                      Props.InstanceName, DiskHandle);
    ...
    case IOCTL_WNBD_FETCH_REQ:
        ...
        if (!Device || !RPAcquired) {
            ...
            WNBD_LOG_ERROR(
                "IOCTL_WNBD_FETCH_REQ: Could not fetch request, invalid disk handle: %d.",
                ReqCmd->DiskHandle);
            break;
        }
    ...
    case IOCTL_WNBD_SEND_RSP:
        ...
        if (!Device || !RPAcquired) {
            Status = STATUS_INVALID_HANDLE;
            WNBD_LOG_ERROR(
                "IOCTL_WNBD_SEND_RSP: Could not fetch request, invalid disk handle: %d.",
                RspCmd->DiskHandle);
            break;
        }
