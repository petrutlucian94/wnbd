    ...
    BOOLEAN ReadOnly)
{
    ...
    Status = WnbdIoctlCreate(WnbdDriverHandle, &Props, &DiskHandle);
    if (Status) {
...
{
    DWORD BufferSize = sizeof(WNBD_DRV_STATS);
    WNBD_DRV_STATS Stats = {0};

    DWORD Status = WnbdGetDriverStats(InstanceName, &Stats, &BufferSize);
    if (Status) {
        CheckOpenFailed(Status);
